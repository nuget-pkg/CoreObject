...
using Global;
public
    class ObjectConverter : IConvertParsedResult {
    ...
    public ObjectConverter(IParseJson? jsonParser = null,
...
internal class JsonStringBuilder {
    ...
    private readonly ObjectConverter _poc;
    private readonly bool _removeSurrogatePair;
    private readonly bool _sortKeys /*= false*/;
    public JsonStringBuilder(ObjectConverter poc, bool forceAscii, bool indentJson, bool sortKeys,
    ...
    public void WriteToSb(StringBuilder sb, object? x, int level, bool cancelIndent = false, bool noQuoteKey = false) {
        ...
        if (type == typeof(string) || type == typeof(char)) {
            ...
            if (noQuoteKey) {
                if (ObjectConverter.IsValidSymbolName(str)) {
                    sb.Append(str);
                    return;
                }
            }
